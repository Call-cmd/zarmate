...
const handleIncomingMessage = async (req, res) => {
  // Telegram sends a nested payload
  const { message } = req.body;
  if (!message) return res.sendStatus(200); // Acknowledge the request

  const fromChatId = message.chat.id;
  const text = message.text || "";
  const fromUserHandle = message.from.username; // e.g., 'johndoe'
  ...
  if (!sender) {
    await telegram.sendMessage(fromChatId, "Sorry, your Telegram handle is not registered with ZarMate.");
...
if (claimMatch) {
  ...
  const paymentMatch = msg.match(/^pay\s+([\w-]+)/i); // Simplified regex
  if (paymentMatch) {
